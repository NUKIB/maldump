...
class EsetNdfParser(KaitaiStruct):
    ...
    class Threat(KaitaiStruct):
        ...
        def _read(self):
            ...
            if not self.date_block_header == b"\x4E\x49\x57\x49":
                raise kaitaistruct.ValidationNotEqualError(b"\x4E\x49\x57\x49", self.date_block_header, self._io, u"/types/threat/seq/2")
            ...
            if not self.filler1 == b"\x00\x00\x00\x00":
                raise kaitaistruct.ValidationNotEqualError(b"\x00\x00\x00\x00", self.filler1, self._io, u"/types/threat/seq/8")
            ...
            if not self.filler2 == b"\x00\x00\x00\x00":
                raise kaitaistruct.ValidationNotEqualError(b"\x00\x00\x00\x00", self.filler2, self._io, u"/types/threat/seq/11")
    ...
    class Widestr(KaitaiStruct):
        ...
        def _read(self):
            ...
            self.str_cont = (self._io.read_bytes((2 * self.str_len))).decode(u"UTF-16LE")
